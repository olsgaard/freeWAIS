...
/*
 * $Log: myAsciiSrc.c,v $
 * Revision 2.0.1.6  1995/12/06 11:10:55  pfeifer
 * patch53: Added inclusion of "../ir/cdialect.h".
 *
 * Revision 2.0.1.5  1995/11/14  18:36:41  pfeifer
 * patch51:
 *
 * Revision 2.0.1.4  1995/10/20  17:10:22  pfeifer
 * patch40: Fixed the sys_errlist stuff (for the last time?).
 *
 * Revision 2.0.1.3  1995/10/10  19:03:49  pfeifer
 * patch35: Not all compilers know elseif :-(
 *
 * Revision 2.0.1.2  1995/10/06  07:27:02  pfeifer
 * patch22: Added sys_errlist hack for BSD.
 *
 * Revision 2.0.1.1  1995/09/15  09:56:50  pfeifer
 * patch7: Fixed CNIDR copyright line.
 *
 * Revision 2.0  1995/09/08  08:06:50  pfeifer
 * Metaconfig baseline
 *
 * Revision 1.4  1994/03/25  10:21:02  pfeifer
 * now will try to make 8-bit chars work
 *
 * Revision 1.2  93/07/01  18:19:07  warnock
 * Fixed declaration in MyAsciiSaveAsFile for SGI/X11R4
 * 
 * Revision 1.1  93/02/16  15:10:18  freewais
 * Initial revision
 * 
 * Revision 1.1  92/06/22  10:42:19  jonathan
 * Initial revision
 * 
 */
...
#define offset(field) XtOffsetOf(MyAsciiSrcRec, ascii_src.field)
...
/* core_class fields */	
    ...
    /* class_name	  	*/	"myAsciiSrc",
    /* widget_size	  	*/	sizeof(MyAsciiSrcRec),
...
WidgetClass myasciiSrcObjectClass = (WidgetClass)&myasciiSrcClassRec;
...
Widget request, new;
{
  MyAsciiSrcObject src = (MyAsciiSrcObject) new;
  FILE * file;
...
int length;		
{
  MyAsciiSrcObject src = (MyAsciiSrcObject) w;
...
XawTextBlock *        text;
{
  ...
  while (TRUE) {
    if (tolower(*ptr) == tolower(((dir == XawsdRight) ? *(buf + count) 
...
{
  MyAsciiSrcObject src =      (MyAsciiSrcObject) new;
  MyAsciiSrcObject old_src = (MyAsciiSrcObject) current;
...
Widget w;
{
  RemoveOldStringOrFile((MyAsciiSrcObject) w, True);
}
...
/*	Function Name: MyAsciiSourceFreeString
...
MyAsciiSourceFreeString(Widget w)
#else
MyAsciiSourceFreeString(w)
Widget w;
#endif
{
  MyAsciiSrcObject src = (MyAsciiSrcObject) w;
...
#if NeedFunctionPrototypes
#ifdef sgi
MyAsciiSaveAsFile(Widget w, String name)
#else
MyAsciiSaveAsFile(Widget w, _Xconst char* name)
#endif
#else
MyAsciiSaveAsFile(w, name)
Widget w;
String name;
#endif
{
...
Boolean 
#if NeedFunctionPrototypes
MyAsciiSourceChanged(Widget w)
#else
MyAsciiSourceChanged(w)
Widget w;
#endif
{
  return( ((MyAsciiSrcObject) w)->ascii_src.changes );
}
...
Widget
MyStringSourceCreate(parent, args, num_args)
Widget parent;
ArgList args;
Cardinal num_args;
{
  ...
  src = XtCreateWidget("genericAsciiString", myasciiSrcObjectClass, parent,
...
void 
MyTextSetLastPos (w, lastPos)
Widget w;
XawTextPosition lastPos;
{
  MyAsciiSrcObject src = (MyAsciiSrcObject) XawTextGetSource(w);
...
Widget
MyDiskSourceCreate(parent, args, num_args)
Widget parent;
ArgList args;
Cardinal num_args;
{
  ...
  src = XtCreateWidget("genericAsciiDisk", myasciiSrcObjectClass, parent,
		       ascii_args, num_args);
  XtFree((char *)ascii_args);
  return(src);
}
